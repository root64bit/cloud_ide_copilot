...
if (!repository || !repository.includes("/")) {
  console.error("OPENHANDS_DIRECT_VERIFY=FAIL reason=OPENHANDS_TEST_REPOSITORY_missing expected=owner/repository");
  process.exit(1);
}
...
console.log(`OPENHANDS_DIRECT_VERIFY_REPOSITORY=${repository}`);
...
if (!startResp.ok) {
  console.error(`OPENHANDS_START=FAIL status=${startResp.status}`);
  process.exit(1);
}
...
while (!conversationId && Date.now() < startDeadline) {
  ...
  if (task.status === "ERROR") {
    console.error("OPENHANDS_START=FAIL reason=start_task_error");
    process.exit(1);
  }
  if (task.app_conversation_id) conversationId = task.app_conversation_id;
}
...
while (Date.now() < execDeadline) {
  ...
  if (convResp.ok) {
    ...
    if (conv) {
      finalStatus = conv.execution_status || conv.status || "running";
      console.log(`OPENHANDS_EXECUTION_STATUS=${finalStatus}`);
      if (["finished", "error", "stuck", "waiting_for_confirmation", "COMPLETED", "FAILED"].includes(finalStatus)) break;
    }
  }
  await new Promise((resolve) => setTimeout(resolve, 5000));
}

if (!["finished", "COMPLETED"].includes(finalStatus)) {
  console.error(`OPENHANDS_LIVE_API_STATUS=FAIL status=${finalStatus}`);
  process.exit(1);
}
